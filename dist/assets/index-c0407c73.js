 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Aj{constructor(e){this.app=e}_delete(){return delete ko[this.app.options.appId],Promise.resolve()}}let ko={},l_=[];const c_={};let Wd="dataLayer",Rj="gtag",u_,YI,d_=!1;function Cj(){const t=[];if(Xw()&&t.push("This is a browser extension environment."),yR()||t.push("Cookies are not available."),t.length>0){const e=t.map((r,i)=>`(${i+1}) ${r}`).join(" "),n=Bt.create("invalid-analytics-context",{errorInfo:e});Dt.warn(n.message)}}function Pj(t,e,n){Cj();const r=t.options.appId;if(!r)throw Bt.create("no-app-id");if(!t.options.apiKey)if(t.options.measurementId)Dt.warn(`The "apiKey" field is empty in the local Firebase config. This is needed to fetch the latest measurement ID for this Firebase app. Falling back to the measurement ID ${t.options.measurementId} provided in the "measurementId" field in the local Firebase config.`);else throw Bt.create("no-api-key");if(ko[r]!=null)throw Bt.create("already-exists",{id:r});if(!d_){lj(Wd);const{wrappedGtag:s,gtagCore:o}=hj(ko,l_,c_,Wd,Rj);YI=s,u_=o,d_=!0}return ko[r]=Sj(t,l_,c_,e,u_,Wd,n),new Aj(t)}function kj(t=ga()){t=pe(t);const e=Vr(t,Vc);return e.isInitialized()?e.getImmediate():Nj(t)}function Nj(t,e={}){const n=Vr(t,Vc);if(n.isInitialized()){const i=n.getImmediate();if(Xo(e,n.getOptions()))return i;throw Bt.create("already-initialized")}return n.initialize({options:e})}function bj(t,e,n,r){t=pe(t),Ij(YI,ko[t.app.options.appId],e,n,r).catch(i=>Dt.error(i))}const h_="@firebase/analytics",f_="0.10.10";function Dj(){dn(new Jt(Vc,(e,{options:n})=>{const r=e.getProvider("app").getImmediate(),i=e.getProvider("installations-internal").getImmediate();return Pj(r,i,n)},"PUBLIC")),dn(new Jt("analytics-internal",t,"PRIVATE")),bt(h_,f_),bt(h_,f_,"esm2017");function t(e){try{const n=e.getProvider(Vc).getImmediate();return{logEvent:(r,i,s)=>bj(n,r,i,s)}}catch(n){throw Bt.create("interop-component-reg-failed",{reason:n})}}}Dj();const Oj={apiKey:"AIzaSyBBsURQuLWU4EmWOMPkvH9WruxsySKNDik",authDomain:"shoppehandloom-7a74d.firebaseapp.com",projectId:"shoppehandloom-7a74d",storageBucket:"shoppehandloom-7a74d.firebasestorage.app",messagingSenderId:"417806513636",appId:"1:417806513636:web:9fa6d656d02bd9101e83f1",measurementId:"G-6TQVLNBBEP"},Du=AC().length?ga():rE(Oj),tt=wu(Du),ro=tD(Du),mr=dO(Du);kj(Du);const XI=D.createContext(),Ri=()=>D.useContext(XI),jj=({children:t})=>{const[e,n]=D.useState(null),[r,i]=D.useState(null),[s,o]=D.useState(!0),[l,c]=D.useState(null);D.useEffect(()=>$b(ro,async N=>{if(n(N),N){const k=await Ai(ht(tt,"users",N.uid));k.exists()?i(k.data()):i(null)}else i(null);o(!1)}),[]);const v={currentUser:e,userData:r,register:async(C,N)=>{c(null);try{const T=(await Fb(ro,C,N)).user;await af(ht(tt,"users",T.uid),{uid:T.uid,email:T.email||null,isAnonymous:!1,createdAt:new Date})}catch(k){throw console.error("Registration Error:",k),c(k.message),k}},login:async(C,N)=>{c(null);try{await Ub(ro,C,N)}catch(k){throw console.error("Login Error:",k),c(k.message),k}},loginAsGuest:async()=>{c(null);try{const N=(await jb(ro)).user;await af(ht(tt,"users",N.uid),{uid:N.uid,email:null,isAnonymous:!0,createdAt:new Date})}catch(C){throw console.error("Anonymous Login Error:",C),c(C.message),C}},logout:async()=>{c(null);try{await qb(ro)}catch(C){throw console.error("Logout Error:",C),c(C.message),C}},authError:l};return d.jsx(XI.Provider,{value:v,children:!s&&t})},Lj=async(t,e,n)=>{try{if(!t.name||!t.price||!t.category||!t.description)throw new Error("All product fields must be filled.");if(!t.variants||t.variants.length===0)throw new Error("At least one variant is required.");for(const l of t.variants){if(!l.size||!l.color)throw new Error("Each variant must have a size and a color.");if(l.stock<0)throw new Error("Stock for each variant cannot be negative.")}let r="",i=[];if(e){const l=pr(mr,`products/${Date.now()}_${e.name}`),c=await Dc(l,e);r=await Oc(c.ref)}if(n&&n.length>0){const l=n.map(async c=>{const u=pr(mr,`products/${Date.now()}_${c.name}`),f=await Dc(u,c);return await Oc(f.ref)});i=await Promise.all(l)}const s={name:t.name,price:parseFloat(t.price),category:t.category,description:t.description,variants:t.variants.map(l=>({id:l.id,size:l.size,color:l.color,stock:l.stock})),primaryImage:r,additionalImages:i,createdAt:xe.now(),updatedAt:xe.now()};return(await Cx(Si(tt,"products"),s)).id}catch(r){throw console.error("Error adding product: ",r),r}},No=async()=>{try{const t=Si(tt,"products");return(await Su(t)).docs.map(r=>({id:r.id,...r.data()}))}catch(t){throw console.error("Error fetching products: ",t),t}},ca=async t=>{try{const e=ht(tt,"products",t),n=await Ai(e);return n.exists()?{id:n.id,...n.data()}:(console.error(`Product with ID ${t} not found.`),null)}catch(e){throw console.error("Error fetching product:",e),e}},Vj=async(t,e)=>{try{const n=wu();return await kx(n,async r=>{const i=[];for(const l of e.cartItems){const c=ht(n,"products",l.productId),u=await r.get(c);if(!u.exists())throw new Error(`Product ${l.productId} not found`);const f=u.data(),p=f.variants.findIndex(C=>C.id===l.variantId);if(p===-1)throw new Error(`Variant ${l.variantId} not found in product ${l.productId}`);const g=f.variants[p].stock;if(g<l.quantity)throw new Error(`Insufficient stock for ${f.name} - ${l.color} ${l.size}`);const v=[...f.variants];v[p]={...v[p],stock:g-l.quantity},i.push({ref:c,variants:v})}const s=Si(n,"orders"),o=ht(s);return r.set(o,{...e,id:o.id,userId:t,createdAt:xe.now(),status:"Processing"}),i.forEach(l=>{r.update(l.ref,{variants:l.variants})}),o.id})}catch(n){throw console.error("Error processing order:",n),n}},Mj=async t=>{try{const e=Si(tt,"orders"),n=Hp(e,LN("userId","==",t),Qp("createdAt","desc")),r=await Su(n),i=[];return r.forEach(s=>{i.push({id:s.id,...s.data()})}),i}catch(e){throw console.error("Error fetching user orders:",e),e}},JI=async t=>{try{const e=ht(tt,"orders",t),n=await Ai(e);return n.exists()?{id:n.id,...n.data()}:(console.error(`Order with ID ${t} not found.`),null)}catch(e){throw console.error("Error fetching order:",e),e}},Fj=async(t,e)=>{try{const n=Si(tt,"users",t,"favourites");return(await Cx(n,{productId:e,addedAt:xe.now()})).id}catch(n){throw console.error("Error adding favourite:",n),n}},Uj=async t=>{try{const e=Si(tt,"users",t,"favourites"),n=Hp(e,Qp("addedAt","desc"));return(await Su(n)).docs.map(i=>({id:i.id,...i.data()}))}catch(e){throw console.error("Error fetching favourites:",e),e}},Bj=async(t,e)=>{try{const n=ht(tt,"users",t);await af(n,{addressInfo:e},{merge:!0})}catch(n){throw console.error("Error saving user address:",n),n}},zj=async(t,e,n=null,r=[])=>{try{const i=ht(tt,"products",t),s=await Ai(i);if(!s.exists())throw new Error("Product not found.");const o=s.data();if(!e.name||!e.price||!e.category||!e.description)throw new Error("All product fields must be filled.");if(!e.variants||e.variants.length===0)throw new Error("At least one variant is required.");for(const f of e.variants){if(!f.size||!f.color)throw new Error("Each variant must have a size and a color.");if(f.stock<0)throw new Error("Stock for each variant cannot be negative.")}let l=o.primaryImage,c=o.additionalImages||[];if(n){if(o.primaryImage){const g=pr(mr,o.primaryImage);await jc(g).catch(v=>{console.error("Error deleting old primary image:",v)})}const f=pr(mr,`products/${Date.now()}_${n.name}`),p=await Dc(f,n);l=await Oc(p.ref)}if(r.length>0){if(c&&c.length>0){const p=c.map(async g=>{const v=pr(mr,g);await jc(v).catch(C=>{console.error("Error deleting old additional image:",C)})});await Promise.all(p)}const f=r.map(async p=>{const g=pr(mr,`products/${Date.now()}_${p.name}`),v=await Dc(g,p);return await Oc(v.ref)});c=await Promise.all(f)}const u={name:e.name,price:parseFloat(e.price),category:e.category,description:e.description,variants:e.variants.map(f=>({id:f.id,size:f.size,color:f.color,stock:f.stock})),primaryImage:l,additionalImages:c,updatedAt:xe.now()};await Rx(i,u)}catch(i){throw console.error("Error updating product: ",i),i}},$j=async t=>{try{const e=ht(tt,"products",t),n=await Ai(e);if(!n.exists())throw new Error("Product not found");const r=n.data();if(r.primaryImage){const i=pr(mr,r.primaryImage);await jc(i).catch(s=>{console.error("Error deleting primary image:",s)})}if(r.additionalImages&&r.additionalImages.length>0){const i=r.additionalImages.map(async s=>{const o=pr(mr,s);await jc(o).catch(l=>{console.error("Error deleting additional image:",l)})});await Promise.all(i)}await FN(e)}catch(e){throw console.error("Error deleting product:",e),e}},qj=(t,e)=>{const n=wu(),r=ht(n,"products",t);return UN(r,i=>{i.exists()?e({id:i.id,...i.data()}):e(null)},i=>{console.error("Error subscribing to product updates:",i)})},_l=async(t,e,n)=>{const r=wu();try{await kx(r,async i=>{const s=ht(r,"products",t),o=await i.get(s);if(!o.exists())throw new Error("Product not found");const l=o.data(),c=l.variants.findIndex(g=>g.id===e);if(c===-1)throw new Error("Variant not found");const f=l.variants[c].stock+n;if(f<0)throw new Error("Insufficient stock");const p=[...l.variants];p[c]={...p[c],stock:f},i.update(s,{variants:p})})}catch(i){throw console.error("Error updating stock:",i),i}};var ZI={exports:{}},Wj="SECRET_DO_NOT_PASS_THIS_OR_YOU_WILL_BE_FIRED",Hj=Wj,Gj=Hj;function eT(){}function tT(){}tT.resetWarningCache=eT;var Kj=function(){function t(r,i,s,o,l,c){if(c!==Gj){var u=new Error("Calling PropTypes validators directly is not supported by the `prop-types` package. Use PropTypes.checkPropTypes() to call them. Read more at http://fb.me/use-check-prop-types");throw u.name="Invariant Violation",u}}t.isRequired=t;function e(){return t}var n={array:t,bigint:t,bool:t,func:t,number:t,object:t,string:t,symbol:t,any:t,arrayOf:e,element:t,elementType:t,instanceOf:e,node:t,objectOf:e,oneOf:e,oneOfType:e,shape:e,exact:e,checkPropTypes:tT,resetWarningCache:eT};return n.PropTypes=n,n};ZI.exports=Kj();var Qj=ZI.exports;const Yj=y_(Qj),nT=D.createContext(),vm=()=>D.useContext(nT),rT=({children:t})=>{const[e,n]=D.useState([]),[r,i]=D.useState(!1),[s,o]=D.useState(null);D.useEffect(()=>{try{const p=localStorage.getItem("cart");p&&n(JSON.parse(p))}catch(p){console.error("Error loading cart from localStorage:",p)}},[]),D.useEffect(()=>{try{localStorage.setItem("cart",JSON.stringify(e))}catch(p){console.error("Error saving cart to localStorage:",p)}},[e]);const l=async(p,g=1,v,C)=>{i(!0),o(null);try{const N=p.variants.find(V=>V.size===v&&V.color===C);if(!N)throw new Error("Selected variant does not exist.");const k=N.id,T=await ca(p.id);if(!T)throw new Error("Product does not exist.");const E=T.variants.find(V=>V.id===k);if(!E)throw new Error("Variant does not exist.");const R=E.stock;if(R===0)throw new Error("This variant is currently out of stock.");const O=e.find(V=>V.variantId===k);if((O?O.quantity+g:g)>R)throw new Error(`Only ${R} items available for this variant.`);await _l(p.id,k,-g),n(V=>O?V.map(I=>I.variantId===k?{...I,quantity:I.quantity+g}:I):[...V,{variantId:k,productId:p.id,name:p.name,price:p.price,size:v,color:C,quantity:g,image:p.primaryImage}])}catch(N){throw console.error("Error adding to cart:",N),o(N.message),N}finally{i(!1)}},c=async p=>{try{const g=e.find(v=>v.variantId===p);g&&(await _l(g.productId,p,g.quantity),n(v=>v.filter(C=>C.variantId!==p)))}catch(g){console.error("Error removing from cart:",g),o(g.message)}},u=async(p,g)=>{if(g<1){o("Quantity cannot be less than 1.");return}i(!0),o(null);try{const v=e.find(T=>T.variantId===p);if(!v)throw new Error("Cart item does not exist.");const C=await ca(v.productId);if(!C)throw new Error("Product does not exist.");if(!C.variants.find(T=>T.id===p))throw new Error("Variant does not exist.");const k=g-v.quantity;await _l(v.productId,p,-k),n(T=>T.map(E=>E.variantId===p?{...E,quantity:g}:E))}catch(v){console.error("Error updating cart quantity:",v),o(v.message)}finally{i(!1)}},f=async()=>{try{for(const p of e)await _l(p.productId,p.variantId,p.quantity);n([])}catch(p){console.error("Error clearing cart:",p),o(p.message)}};return d.jsx(nT.Provider,{value:{cartItems:e,addToCart:l,removeFromCart:c,updateQuantity:u,clearCart:f,loading:r,error:s},children:t})};rT.propTypes={children:Yj.node.isRequired};const Xj=({onClose:t})=>{const{register:e,login:n,loginAsGuest:r,authError:i}=Ri(),[s,o]=D.useState(!0),[l,c]=D.useState(""),[u,f]=D.useState(""),[p,g]=D.useState(!1),v=async N=>{N.preventDefault(),g(!0);try{s?await e(l,u):await n(l,u),t()}catch{}finally{g(!1)}},C=async()=>{g(!0);try{await r(),t()}catch{}finally{g(!1)}};return d.jsx("div",{className:"fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center",children:d.jsxs("div",{className:"bg-white rounded-lg p-6 w-96",children:[d.jsx("h2",{className:"text-2xl font-bold mb-4",children:s?"Register":"Login"}),i&&d.jsx("div",{className:"mb-4 text-red-500",children:i}),d.jsxs("form",{onSubmit:v,className:"space-y-4",children:[d.jsxs("div",{children:[d.jsx("label",{htmlFor:"email",className:"block text-gray-700",children:"Email"}),d.jsx("input",{type:"email",id:"email",value:l,onChange:N=>c(N.target.value),required:!0,className:"w-full px-3 py-2 border rounded",placeholder:"Enter your email"})]}),d.jsxs("div",{children:[d.jsx("label",{htmlFor:"password",className:"block text-gray-700",children:"Password"}),d.jsx("input",{type:"password",id:"password",value:u,onChange:N=>f(N.target.value),required:!0,className:"w-full px-3 py-2 border rounded",placeholder:"Enter your password"})]}),d.jsx("button",{type:"submit",disabled:p,className:`w-full bg-[#6F3A19] text-white py-2 rounded-md hover:bg-[#8B533B] transition-colors duration-300 ${p?"opacity-50 cursor-not-allowed":""}`,children:p?s?"Registering...":"Logging in...":s?"Register":"Login"})]}),d.jsxs("div",{className:"mt-4 flex items-center justify-between",children:[d.jsx("button",{onClick:()=>o(!s),className:"text-sm text-[#6F3A19] hover:underline",children:s?"Already have an account? Login":"Don't have an account? Register"}),d.jsx("button",{onClick:C,disabled:p,className:`text-sm text-gray-600 hover:underline ${p?"opacity-50 cursor-not-allowed":""}`,children:"Continue as Guest"})]})]})})},iT="/assets/favorite (1)-de17bf8e.svg",Jj="/assets/person-bcddabb0.svg",Zj="/assets/Cart 4-b4cf6982.svg",eL="/assets/Home 1-9c21df8e.svg";function ls(){const t=Gn();ru();const{currentUser:e,userData:n,logout:r}=Ri(),[i,s]=D.useState({textColor:"white"}),[o,l]=D.useState("/"),[c,u]=D.useState(!1),[f,p]=D.useState(!1);D.useEffect(()=>{const k=()=>{const T=document.querySelectorAll("section"),E=window.pageYOffset;let R=0;T.forEach((O,F)=>{const V=O.offsetTop,I=O.offsetHeight;E>=V&&E<V+I&&(R=F)}),R%2===0?s({textColor:"black"}):s({textColor:"white"})};return window.addEventListener("scroll",k,{passive:!0}),()=>window.removeEventListener("scroll",k)},[]);const g=k=>o===k?{borderRadius:"63px",padding:"0 10px",border:"2px solid #6F3A19",transition:"width 0.3s"}:{},v=()=>{e?p(!f):u(!0)},C=async()=>{try{await r(),t("/")}catch(k){console.error("Logout Error:",k)}},N=[{label:"Profile",path:"/profile"},{label:"Orders",path:"/orders"},{label:"Favorites",path:"/favorites"},{label:"Settings",path:"/settings"}];return d.jsxs("header",{className:"fixed top-0 left-0 right-0 z-50 flex flex-col lg:flex-row items-center justify-between px-5 py-3 transition-all duration-300",style:{backgroundColor:"transparent"},children:[d.jsxs("div",{className:"w-full flex items-center justify-between",children:[d.jsxs("div",{className:"flex items-center space-x-4",children:[d.jsx("button",{onClick:()=>{l("/"),t("/")},style:g("/"),className:"flex items-center focus:outline-none",children:d.jsx("img",{src:eL,alt:"Home",className:"w-6 h-6"})}),d.jsx("button",{onClick:()=>{l("/products"),t("/products")},style:g("/products"),className:"focus:outline-none",children:d.jsx("span",{className:"text-sm hedvig",style:{color:i.textColor},children:"Products"})}),d.jsx("button",{onClick:()=>{l("/about"),t("/about")},style:g("/about"),className:"focus:outline-none",children:d.jsx("span",{className:"text-sm hedvig",style:{color:i.textColor},children:"About"})})]}),d.jsxs("div",{className:"flex items-center space-x-4",children:[d.jsxs("button",{onClick:()=>t("/favorites"),className:"relative focus:outline-none",children:[d.jsx("img",{src:iT,alt:"Favorite",className:"w-6 h-6"}),e&&(n==null?void 0:n.favouritesCount)>0&&d.jsx("span",{className:"absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center",children:n.favouritesCount})]}),d.jsx("button",{onClick:()=>t("/cart"),className:"focus:outline-none",children:d.jsx("img",{src:Zj,alt:"Cart",className:"w-6 h-6"})}),d.jsxs("div",{className:"relative",children:[d.jsx("button",{onClick:v,className:"focus:outline-none",children:e?d.jsx("div",{className:"w-6 h-6 rounded-full bg-[#6F3A19] flex items-center justify-center",children:d.jsx("span",{className:"text-white text-sm",children:n!=null&&n.email?n.email.charAt(0).toUpperCase():"G"})}):d.jsx("img",{src:Jj,alt:"Person",className:"w-6 h-6"})}),e&&f&&d.jsxs("div",{className:"absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-10",children:[N.map((k,T)=>d.jsx("button",{onClick:()=>t(k.path),className:"block w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 text-left",children:k.label},T)),d.jsx("hr",{className:"my-1"}),d.jsx("button",{onClick:C,className:"block w-full px-4 py-2 text-sm text-red-600 hover:bg-gray-100 text-left",children:"Sign Out"})]})]})]})]}),d.jsx("a",{href:"/",className:"hidden lg:block text-4xl font-kalam transition-colors duration-300 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2",style:{color:"#6F3A19",WebkitTextStroke:"1px #6F4000"},children:"मोहरी"}),d.jsx("a",{href:"/",className:"lg:hidden text-3xl font-kalam mt-3 text-center text-[#6F3A19] transition-colors duration-300",style:{WebkitTextStroke:"1px #6F4000"},children:"मोहरी"}),c&&d.jsx(Xj,{onClose:()=>u(!1)})]})}const p_="/assets/Group 7-1f58af99.svg",xt=()=>d.jsxs("footer",{className:"footer h-full",children:[d.jsxs("div",{className:"footer-top",children:[d.jsx("p",{children:"Can sheets and towels be washed together?"}),d.jsx("button",{className:"footer-button",children:"read article"})]}),d.jsxs("div",{className:"footer-links",children:[d.jsxs("ul",{children:[d.jsx("li",{children:"About Sonette"}),d.jsx("li",{children:"Store"}),d.jsx("li",{children:"Gift Card"})]}),d.jsxs("ul",{children:[d.jsx("li",{children:"Contact us"}),d.jsx("li",{children:"Privacy Policy"}),d.jsx("li",{children:"Terms and Conditions"}),d.jsx("li",{children:"Legal Notice"})]}),d.jsxs("ul",{children:[d.jsx("li",{children:"Our guides"}),d.jsx("li",{children:"Choosing your bedding"}),d.jsx("li",{children:"Our expertise"})]})]}),d.jsx("div",{className:"footer-bottom",children:d.jsx("h1",{children:"मोहारी"})})]}),tL="/assets/favorite-51b9de98.svg",sT=({product:t,onClick:e})=>{const{currentUser:n}=Ri();vm();const[r,i]=D.useState(!1),[s,o]=D.useState(!1),[l,c]=D.useState(null);D.useEffect(()=>{(async()=>{if(n)try{const g=(await Uj(n.uid)).find(v=>v.productId===t.id);i(!!g)}catch(p){console.error("Error fetching favourites:",p)}})()},[n,t.id]);const u=async f=>{if(f.stopPropagation(),t.stock===0){c("Cannot add out-of-stock product to favourites.");return}if(!n){c("Please log in to add favourites.");return}if(r){c("This product is already in your favourites.");return}o(!0),c(null);try{await Fj(n.uid,t.id),i(!0)}catch(p){console.error("Error adding to favourites:",p),c("Failed to add to favourites.")}finally{o(!1)}};return d.jsxs("div",{onClick:e,className:"bg-white rounded-lg shadow-lg w-[200px] h-[250px] text-left overflow-hidden m-2 flex flex-col items-center hover:shadow-2xl transition-shadow duration-300 cursor-pointer",children:[d.jsx("img",{src:t.primaryImage||"https://via.placeholder.com/200x175",alt:t.name,className:"w-full h-[70%] object-cover"}),d.jsxs("div",{className:"p-2 flex flex-col w-full h-[30%]",children:[d.jsx("div",{className:"flex items-center h-[60%]",children:d.jsx("h3",{className:"text-lg font-kalam truncate",children:t.name})}),d.jsxs("div",{className:"flex items-center justify-between h-[40%]",children:[d.jsxs("p",{className:"text-gray-700 font-hedvig",children:["₹",t.price.toFixed(2)]}),d.jsx("button",{className:`bg-white border ${t.stock===0?"border-gray-400":"border-[#6F3A19]"} rounded p-1 hover:bg-gray-100 transition-colors duration-200 ${t.stock===0?"cursor-not-allowed":"cursor-pointer"}`,onClick:u,"aria-label":"Add to Favourite",disabled:s||r||t.stock===0,children:d.jsx("img",{src:r?tL:iT,alt:r?"Favourite Filled":"Add to Favourite",className:"w-5 h-5"})})]}),l&&d.jsx("p",{className:"text-red-500 text-sm mt-1",children:l}),t.stock===0&&d.jsx("p",{className:"text-red-500 text-sm mt-1",children:"Out of Stock"})]})]})},nL="/assets/Designer-315668df.jpeg",rL="/assets/vid-f6e5b122.mp4";function iL(){const[t,e]=D.useState(""),[n,r]=D.useState([]),[i,s]=D.useState([]),[o,l]=D.useState(!0),[c,u]=D.useState(null),f=D.useRef(null),p=Gn();D.useEffect(()=>{(async()=>{try{const k=await No();r(k),s(k.slice(0,4))}catch(k){console.error("Error fetching products:",k),u("Failed to load products")}finally{l(!1)}})()},[]);const g=N=>{if(N){const k=n.filter(T=>T.category.toLowerCase()===N.toLowerCase());s(k.slice(0,4))}else s(n.slice(0,4))};D.useEffect(()=>{g(t)},[t,n]),D.useEffect(()=>{const N=document.querySelectorAll("section"),k=()=>{let E=null,R=1/0;N.forEach(O=>{const F=O.getBoundingClientRect(),V=Math.abs(F.top);V<R&&(R=V,E=O)}),E&&E.scrollIntoView({behavior:"smooth",block:"start"})},T=(()=>{let E;return()=>{clearTimeout(E),E=setTimeout(k,100)}})();return window.addEventListener("scroll",T,{passive:!0}),()=>{window.removeEventListener("scroll",T)}},[]);const v=N=>{e(N),f.current&&f.current.scrollIntoView({behavior:"smooth"})},C=()=>{p("/products")};return d.jsxs("div",{className:"space-y-4 md:space-y-8",children:[d.jsx("section",{className:"relative h-[80vh] md:h-screen bg-cover bg-center flex items-center justify-center",style:{backgroundImage:`url(${nL})`,backgroundPosition:"center center"},children:d.jsxs("div",{className:"absolute inset-0 flex flex-col items-center justify-center text-center p-4 md:p-8",children:[d.jsx("h1",{className:"hedvig text-3xl md:text-5xl lg:text-[60px] mb-4 leading-tight md:leading-tight lg:leading-tight",children:"Sustainable Clothing"}),d.jsx("button",{className:"footer-button text-xs md:text-sm px-4 py-2",onClick:C,children:"Shop Now"})]})}),d.jsxs("section",{className:"relative h-[60vh] md:h-[80vh] flex items-end",children:[d.jsx("video",{className:"absolute inset-0 w-full h-full object-cover z-0",autoPlay:!0,loop:!0,muted:!0,playsInline:!0,children:d.jsx("source",{src:rL,type:"video/mp4"})}),d.jsx("div",{className:"w-full flex justify-center items-end p-4 md:p-10 gap-2 md:gap-4 z-10",children:["Duvets","Bedding","Pillow","Mattress","Sheets"].map((N,k)=>d.jsx("span",{className:"text-lg md:text-2xl lg:text-4xl hedvig transition-all duration-300 opacity-20 hover:text-[#6F3A19] hover:opacity-100 hover:scale-125 cursor-pointer mx-2",onClick:()=>v(N.toLowerCase()),children:N},k))})]}),d.jsxs("section",{ref:f,className:"min-h-screen bg-gray-100 px-4 py-8 md:py-12",children:[d.jsxs("h1",{className:"text-2xl md:text-3xl hedvig text-center mb-6",children:["Featured ",t?`- ${t.charAt(0).toUpperCase()+t.slice(1)}`:""]}),o?d.jsx("div",{className:"text-center",children:"Loading products..."}):c?d.jsx("div",{className:"text-center text-red-500",children:c}):d.jsx("div",{className:"grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 justify-items-center",children:i.length>0?i.map(N=>d.jsx(sT,{product:N,onClick:()=>p(`/product/${N.id}`),className:"w-full max-w-[200px]"},N.id)):d.jsx("div",{className:"text-center",children:"No products found."})})]}),d.jsxs("section",{className:"h-[60vh] md:h-[80vh] bg-white flex flex-col items-center justify-center relative overflow-hidden",children:[d.jsx("div",{className:"absolute top-0 left-0 right-0",children:d.jsx("img",{src:p_,alt:"Group 7",className:"w-full object-cover",style:{maxHeight:"30vh",maxWidth:"100vw"}})}),d.jsxs("div",{className:"text-center z-10 px-4 md:px-8",children:[d.jsx("h2",{className:"text-xl md:text-4xl hedvig text-[#6F3A19]",children:"Why Mohari"}),d.jsx("p",{className:"mt-2 text-xs md:text-base text-gray-700",children:"Discover the craftsmanship and sustainable practices that make our products unique."})]}),d.jsx("div",{className:"absolute bottom-0 left-0 right-0",children:d.jsx("img",{src:p_,alt:"Group 7",className:"w-full object-cover",style:{maxHeight:"30vh",maxWidth:"100vw"}})})]}),d.jsx(xt,{})]})}const pf="/assets/duvet-d4a5033a.mp4",sL="/assets/bedding-b0632a17.mp4",oL="/assets/mattress-8e67ac8b.mp4",aL="/assets/pillows-f0d2ca1e.mp4",lL="/assets/sheets-8c2d24d8.mp4",cL=["duvets","bedding","mattress","pillow","sheets"],uL={duvet:pf,bedding:sL,mattress:oL,pillow:aL,sheets:lL},m_={duvets:["Single","Double","Queen","King"],bedding:["Twin","Full","Queen","King"],pillow:["Standard","Queen","King"],mattress:["Twin","Full","Queen","King","California King"],sheets:["Twin","Full","Queen","King","California King"]},dL=["Red","Blue","Green","Yellow","Cyan","Magenta"],hL=()=>{const[t,e]=D.useState(!1),[n,r]=D.useState(!1),[i,s]=D.useState(!1),[o,l]=D.useState(""),[c,u]=D.useState(""),[f,p]=D.useState(""),[g,v]=D.useState(pf),[C,N]=D.useState([]),[k,T]=D.useState([]),[E,R]=D.useState(!0),[O,F]=D.useState(null),V=Gn();D.useEffect(()=>{(async()=>{try{const S=await No();N(S),T(S)}catch(S){console.error("Error fetching products:",S),F("Failed to load products")}finally{R(!1)}})()},[]);const I=({category:A,size:S,color:te})=>{let ne=C;A&&(ne=ne.filter(le=>le.category.toLowerCase()===A.toLowerCase())),S&&(ne=ne.filter(le=>le.sizes&&le.sizes.includes(S))),te&&(ne=ne.filter(le=>le.colors&&le.colors.includes(te))),T(ne)},w=A=>{const S=A.charAt(0).toUpperCase()+A.slice(1);l(S),v(uL[A]||pf),u(""),p(""),I({category:A.toLowerCase(),size:"",color:""}),e(!1)},_=A=>{u(A),I({category:o.toLowerCase(),size:A,color:f}),r(!1)},x=A=>{p(A),I({category:o.toLowerCase(),size:c,color:A}),s(!1)},P=A=>{V(`/product/${A}`)};return d.jsxs("div",{children:[d.jsxs("section",{className:"h-screen bg-white flex flex-col items-center justify-center relative",children:[d.jsx("video",{src:g,alt:o,className:"w-full max-w-4xl h-auto object-cover",autoPlay:!0,loop:!0,muted:!0,playsInline:!0}),d.jsx("p",{className:"mt-4 text-center text-sm md:text-base",children:"Explore our featured products that are popular among our customers."}),d.jsx("div",{className:"absolute bottom-0 left-0 w-full max-w-6xl mx-auto p-4",children:d.jsxs("div",{className:"flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4",children:[d.jsx("div",{className:"p-2 cursor-pointer",style:{backgroundColor:"#6F3A19",width:t?"400px":"80px",height:"47px",borderRadius:"63px",paddingLeft:"10px",transition:"width 0.7s ease",display:"flex",justifyContent:t?"space-between":"center",alignItems:"center",overflow:"hidden"},onMouseEnter:()=>e(!0),onMouseLeave:()=>e(!1),children:t?cL.map((A,S)=>d.jsx("button",{className:"text-white mx-1 text-sm md:text-base",onClick:()=>w(A),children:A.charAt(0).toUpperCase()+A.slice(1)},S)):d.jsx("span",{className:"text-white text-sm md:text-base",children:o||"Category"})}),d.jsx("div",{className:"p-2 cursor-pointer",style:{backgroundColor:"#6F3A19",width:n?"275px":"70px",height:"47px",borderRadius:"63px",paddingLeft:"10px",transition:"width 0.7s ease",display:"flex",justifyContent:n?"space-between":"center",alignItems:"center",overflow:"hidden"},onMouseEnter:()=>r(!0),onMouseLeave:()=>r(!1),children:n?o&&m_[o.toLowerCase()]?m_[o.toLowerCase()].map((A,S)=>d.jsx("button",{className:"text-white mx-1 text-sm md:text-base",onClick:()=>_(A),children:A},S)):d.jsx("span",{className:"text-white text-sm md:text-base",children:"N/A"}):d.jsx("span",{className:"text-white text-sm md:text-base",children:c||"Size"})}),d.jsx("div",{className:"p-2 cursor-pointer",style:{backgroundColor:"#6F3A19",width:i?"400px":"80px",height:"47px",borderRadius:"63px",paddingLeft:"10px",transition:"width 0.7s ease",display:"flex",justifyContent:i?"space-between":"center",alignItems:"center",overflow:"hidden"},onMouseEnter:()=>s(!0),onMouseLeave:()=>s(!1),children:i?dL.map((A,S)=>d.jsx("button",{className:"text-white mx-1 text-sm md:text-base",onClick:()=>x(A),children:A},S)):d.jsx("span",{className:"text-white text-sm md:text-base",children:f||"Color"})})]})})]}),d.jsx("section",{className:"flex flex-col items-center justify-center p-4",children:d.jsx("div",{className:"rounded-lg",style:{backgroundColor:"rgba(111, 58, 25, 0.5)",width:"1300px",padding:"20px",margin:"20px 0"},children:d.jsx("div",{className:"grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 justify-items-center",children:E?d.jsx("div",{children:"Loading products..."}):O?d.jsx("div",{className:"text-red-500",children:O}):k.length>0?k.map(A=>d.jsx(sT,{product:A,onClick:()=>P(A.id)},A.id)):d.jsx("div",{children:"No products found."})})})}),d.jsx(xt,{})]})},$l="/assets/Cart 3-a68373f2.svg",fL=()=>{var P;const{id:t}=Uw(),e=Gn(),{addToCart:n,error:r}=vm(),[i,s]=D.useState(null),[o,l]=D.useState(!0),[c,u]=D.useState(null),[f,p]=D.useState(""),[g,v]=D.useState(""),[C,N]=D.useState(!1),[k,T]=D.useState(1),[E,R]=D.useState([]),[O,F]=D.useState([]);D.useEffect(()=>{let A;return(async()=>{try{const te=await ca(t);if(!te){u("Product not found");return}V(te),A=qj(t,ne=>{if(ne){console.log("Product updated:",ne),V(ne);const le=ne.variants.find(St=>St.size===f&&St.color===g);le&&k>le.stock&&T(Math.max(1,le.stock))}})}catch(te){u("Failed to load product details"),console.error(te)}finally{l(!1)}})(),()=>{A&&A()}},[t,f,g,k]);const V=A=>{s(A);const S=[...new Set(A.variants.filter(le=>le.stock>0).map(le=>le.size))],te=[...new Set(A.variants.filter(le=>le.stock>0).map(le=>le.color))];R(S),F(te);const ne=A.variants.find(le=>le.size===f&&le.color===g);ne&&ne.stock===0&&(p(""),v(""),T(1))},I=()=>!f||!g?null:i.variants.find(A=>A.size===f&&A.color===g),w=async()=>{if(!f||!g){alert("Please select both size and color.");return}const A=I();if(!A){alert("Selected variant does not exist.");return}if(A.stock===0){alert("This variant is out of stock.");return}if(k>A.stock){alert(`Only ${A.stock} items available for the selected variant.`);return}try{await n(i,k,f,g),N(!0),setTimeout(()=>N(!1),2e3)}catch(S){console.error("Error adding to cart:",S),alert(r||"Failed to add to cart.")}};if(o)return d.jsx("div",{className:"min-h-screen flex items-center justify-center pt-[100px]",children:d.jsx("div",{className:"animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#6F3A19]"})});if(c||!i)return d.jsx("div",{className:"min-h-screen flex items-center justify-center pt-[100px]",children:d.jsx("p",{className:"text-red-500",children:c||"Product not found"})});const _=()=>{if(!f||!g)return!1;const A=I();return A?A.stock===0:!1},x=()=>{if(!f||!g||!i)return 0;const A=I();return A?A.stock:0};return d.jsxs("div",{className:"min-h-screen",children:[d.jsx(ls,{}),d.jsxs("div",{className:"container mx-auto px-4 py-8 pt-[100px]",children:[d.jsxs("div",{className:"grid grid-cols-1 md:grid-cols-2 gap-8",children:[d.jsxs("div",{className:"space-y-4",children:[d.jsx("div",{className:"aspect-w-1 aspect-h-1 w-full",children:d.jsx("img",{src:i.primaryImage||"https://via.placeholder.com/300x200.png?text=Primary+Image",alt:i.name,className:"object-cover rounded-lg"})}),d.jsx("div",{className:"grid grid-cols-4 gap-2",children:(P=i.additionalImages)==null?void 0:P.map((A,S)=>d.jsx("img",{src:A||"https://via.placeholder.com/100x100.png?text=Image",alt:`${i.name} view ${S+1}`,className:"aspect-square object-cover rounded-lg"},S))})]}),d.jsxs("div",{className:"space-y-6",children:[d.jsx("h1",{className:"text-3xl font-bold text-gray-900",children:i.name}),d.jsxs("p",{className:"text-2xl text-[#6F3A19]",children:["₹",i.price]}),d.jsxs("div",{children:[d.jsx("h2",{className:"text-lg font-semibold mb-2",children:"Description"}),d.jsx("p",{className:"text-gray-600",children:i.description})]}),E&&E.length>0&&d.jsxs("div",{children:[d.jsx("h2",{className:"text-lg font-semibold mb-2",children:"Size"}),d.jsx("div",{className:"flex flex-wrap gap-2",children:E.map(A=>d.jsx("button",{onClick:()=>{p(A),v("")},className:`px-4 py-2 border rounded-md ${f===A?"border-[#6F3A19] bg-[#6F3A19] text-white":"border-gray-300 hover:border-[#6F3A19]"}`,children:A},A))})]}),O&&O.length>0&&d.jsxs("div",{children:[d.jsx("h2",{className:"text-lg font-semibold mb-2",children:"Color"}),d.jsx("div",{className:"flex flex-wrap gap-2",children:O.map(A=>{const S=i.variants.some(te=>te.color===A&&te.size===f&&te.stock>0);return d.jsx("button",{onClick:()=>v(A),disabled:!S,className:`px-4 py-2 border rounded-md ${g===A?"border-[#6F3A19] bg-[#6F3A19] text-white":"border-gray-300 hover:border-[#6F3A19]"} ${S?"":"opacity-50 cursor-not-allowed"}`,title:S?"":"This color is unavailable for the selected size.",children:A},A)})})]}),d.jsxs("div",{children:[d.jsx("label",{htmlFor:"quantity",className:"block text-gray-700 mb-2",children:"Quantity"}),d.jsx("input",{type:"number",id:"quantity",name:"quantity",min:"1",max:x(),value:k,onChange:A=>T(Math.min(x(),Math.max(1,parseInt(A.target.value)||1))),className:"w-20 px-3 py-2 border rounded",disabled:!f||!g}),f&&g&&d.jsxs("p",{className:"text-sm text-gray-600 mt-1",children:["Available Stock: ",x()]})]}),d.jsxs("div",{className:"relative",children:[d.jsx("button",{onClick:w,disabled:_(),className:`w-full bg-[#6F3A19] text-white py-3 px-6 rounded-lg flex items-center justify-center gap-2 relative ${_()?"opacity-50 cursor-not-allowed":"hover:bg-[#8B533B] transition-colors duration-300"}`,children:_()?"Out of Stock":d.jsxs(d.Fragment,{children:[d.jsx("img",{src:$l,alt:"cart",className:"w-6 h-6"}),"Add to Cart"]})}),_()&&d.jsx("div",{className:"absolute inset-0 bg-red-500 bg-opacity-50 flex items-center justify-center rounded-lg",children:d.jsx("span",{className:"text-white font-semibold",children:"Out of Stock"})}),C&&d.jsx("div",{className:"absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 rounded-lg",children:d.jsxs("div",{className:"bg-white shadow-lg rounded-lg p-4 flex flex-col items-center",children:[d.jsx("p",{className:"text-[#6F3A19] font-semibold mb-2",children:"Added to Cart!"}),d.jsx("button",{onClick:()=>e("/cart"),className:"text-sm text-[#6F3A19] hover:underline",children:"View Cart"})]})})]})]})]}),d.jsxs("div",{className:"mt-12",children:[d.jsx("div",{className:"border-b mb-6",children:d.jsx("h2",{className:"text-2xl font-semibold",children:"Details"})}),d.jsxs("div",{className:"space-y-4 mb-12",children:[d.jsxs("p",{children:[d.jsx("strong",{children:"Material:"})," ",i.details.material]}),d.jsxs("p",{children:[d.jsx("strong",{children:"Dimensions:"})," ",i.details.dimensions]}),d.jsxs("p",{children:[d.jsx("strong",{children:"Care Instructions:"})," ",i.details.careInstructions]}),d.jsxs("p",{children:[d.jsx("strong",{children:"Warranty:"})," ",i.details.warranty]})]}),d.jsx("div",{className:"border-b mb-6",children:d.jsx("h2",{className:"text-2xl font-semibold",children:"Reviews"})}),d.jsx("div",{className:"space-y-4",children:i.reviews&&i.reviews.length>0?i.reviews.map(A=>d.jsxs("div",{className:"border p-4 rounded-lg",children:[d.jsx("h3",{className:"text-lg font-semibold",children:A.user}),d.jsxs("p",{className:"text-sm text-gray-500",children:["⭐".repeat(A.rating),"☆".repeat(5-A.rating)]}),d.jsx("p",{className:"mt-2",children:A.comment})]},A.id)):d.jsx("p",{className:"text-gray-600",children:"No reviews yet. Be the first to review!"})})]})]}),d.jsx(xt,{})]})},er="/assets/Image-a8f230c7.jpg",pL=()=>d.jsxs("div",{className:"flex flex-col min-h-screen",children:[d.jsx("div",{className:"container mx-auto px-4 py-8 pt-[25px]"}),d.jsxs("main",{className:"flex-grow bg-white",children:[d.jsxs("section",{className:"relative h-screen bg-cover bg-center",style:{backgroundImage:`url(${er})`},children:[d.jsx("div",{className:"absolute inset-0 bg-black opacity-40"}),d.jsxs("div",{className:"absolute inset-0 flex flex-col items-center justify-center text-center px-4",children:[d.jsx("h1",{className:"text-5xl font-kalam text-white mb-4",children:"About Us"}),d.jsx("p",{className:"text-xl text-gray-200 max-w-2xl",children:"At Mohari, we believe in sustainable and stylish fashion that empowers individuals while respecting the planet. Our mission is to create high-quality clothing that combines comfort, durability, and timeless design."})]})]}),d.jsx("section",{className:"py-16 px-8 bg-gray-100",children:d.jsxs("div",{className:"max-w-6xl mx-auto",children:[d.jsx("h2",{className:"text-3xl font-semibold font-hedvig text-[#6F3A19] mb-6",children:"Our Story"}),d.jsx("p",{className:"text-lg text-gray-700 mb-6",children:"Founded in 2020, Mohari started with a simple yet profound idea: to offer apparel that not only looks good but also feels good. Our founders were inspired by the growing need for sustainable fashion that doesn't compromise on style or quality."}),d.jsx("p",{className:"text-lg text-gray-700",children:"Over the years, we've expanded our range to include a variety of products, each crafted with care and a commitment to ethical practices. From sourcing eco-friendly materials to ensuring fair labor practices, every step of our process is thoughtfully designed to make a positive impact."})]})}),d.jsx("section",{className:"py-16 px-8",children:d.jsxs("div",{className:"max-w-6xl mx-auto",children:[d.jsx("h2",{className:"text-3xl font-semibold font-hedvig text-[#6F3A19] mb-6",children:"Our Values"}),d.jsxs("div",{className:"grid grid-cols-1 md:grid-cols-3 gap-8",children:[d.jsxs("div",{className:"flex flex-col items-center text-center",children:[d.jsx("div",{className:"w-16 h-16 bg-[#6F3A19] bg-opacity-75 rounded-full flex items-center justify-center mb-4",children:d.jsx("svg",{xmlns:"http://www.w3.org/2000/svg",className:"h-8 w-8 text-white",fill:"none",viewBox:"0 0 24 24",stroke:"currentColor",children:d.jsx("path",{strokeLinecap:"round",strokeLinejoin:"round",strokeWidth:2,d:"M12 8c1.657 0 3 1.343 3 3H9c0-1.657 1.343-3 3-3zM5 20h14a2 2 0 002-2V8H3v10a2 2 0 002 2z"})})}),d.jsx("h3",{className:"text-2xl font-kalam mb-2",children:"Sustainability"}),d.jsx("p",{className:"text-gray-700",children:"We prioritize eco-friendly materials and sustainable practices in every aspect of our production."})]}),d.jsxs("div",{className:"flex flex-col items-center text-center",children:[d.jsx("div",{className:"w-16 h-16 bg-[#6F3A19] bg-opacity-75 rounded-full flex items-center justify-center mb-4",children:d.jsx("svg",{xmlns:"http://www.w3.org/2000/svg",className:"h-8 w-8 text-white",fill:"none",viewBox:"0 0 24 24",stroke:"currentColor",children:d.jsx("path",{strokeLinecap:"round",strokeLinejoin:"round",strokeWidth:2,d:"M8 10h.01M12 10h.01M16 10h.01M19 10h.01M5 10h.01M22 12a10 10 0 11-20 0 10 10 0 0120 0z"})})}),d.jsx("h3",{className:"text-2xl font-kalam mb-2",children:"Quality"}),d.jsx("p",{className:"text-gray-700",children:"Our products are crafted with the highest standards to ensure longevity and satisfaction."})]}),d.jsxs("div",{className:"flex flex-col items-center text-center",children:[d.jsx("div",{className:"w-16 h-16 bg-[#6F3A19] bg-opacity-75 rounded-full flex items-center justify-center mb-4",children:d.jsx("svg",{xmlns:"http://www.w3.org/2000/svg",className:"h-8 w-8 text-white",fill:"none",viewBox:"0 0 24 24",stroke:"currentColor",children:d.jsx("path",{strokeLinecap:"round",strokeLinejoin:"round",strokeWidth:2,d:"M15 19l-7-7 7-7"})})}),d.jsx("h3",{className:"text-2xl font-kalam mb-2",children:"Innovation"}),d.jsx("p",{className:"text-gray-700",children:"We embrace creativity and continuously seek new ways to improve our products and processes."})]})]})]})}),d.jsx("section",{className:"py-16 px-8 bg-gray-100",children:d.jsxs("div",{className:"max-w-6xl mx-auto",children:[d.jsx("h2",{className:"text-3xl font-semibold font-hedvig text-[#6F3A19] mb-6",children:"Meet the Team"}),d.jsxs("div",{className:"grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8",children:[d.jsxs("div",{className:"flex flex-col items-center text-center",children:[d.jsx("img",{src:er,alt:"Team Member",className:"w-32 h-32 rounded-full mb-4 object-cover"}),d.jsx("h3",{className:"text-xl font-kalam mb-2",children:"John Doe"}),d.jsx("p",{className:"text-gray-700",children:"Founder & CEO"})]}),d.jsxs("div",{className:"flex flex-col items-center text-center",children:[d.jsx("img",{src:er,alt:"Team Member",className:"w-32 h-32 rounded-full mb-4 object-cover"}),d.jsx("h3",{className:"text-xl font-kalam mb-2",children:"Jane Smith"}),d.jsx("p",{className:"text-gray-700",children:"Chief Designer"})]}),d.jsxs("div",{className:"flex flex-col items-center text-center",children:[d.jsx("img",{src:er,alt:"Team Member",className:"w-32 h-32 rounded-full mb-4 object-cover"}),d.jsx("h3",{className:"text-xl font-kalam mb-2",children:"Emily Johnson"}),d.jsx("p",{className:"text-gray-700",children:"Head of Marketing"})]})]})]})}),d.jsx("section",{className:"py-16 px-8",children:d.jsxs("div",{className:"max-w-6xl mx-auto",children:[d.jsx("h2",{className:"text-3xl font-semibold font-hedvig text-[#6F3A19] mb-6",children:"Our Mission"}),d.jsx("p",{className:"text-lg text-gray-700 mb-6",children:"Our mission is to revolutionize the fashion industry by promoting sustainability, ethical practices, and innovative designs. We aim to create a positive impact on both our customers and the environment, ensuring that every product we offer is a testament to our commitment to excellence and responsibility."}),d.jsx("p",{className:"text-lg text-gray-700",children:"By choosing Mohari, you are not only investing in high-quality apparel but also supporting a brand that values transparency, integrity, and the well-being of our planet. Together, we can make fashion a force for good."})]})}),d.jsx("section",{className:"py-16 px-8 bg-gray-100",children:d.jsxs("div",{className:"max-w-6xl mx-auto",children:[d.jsx("h2",{className:"text-3xl font-semibold font-hedvig text-[#6F3A19] mb-6",children:"What Our Customers Say"}),d.jsxs("div",{className:"space-y-8",children:[d.jsxs("div",{className:"bg-white p-6 rounded-lg shadow-md",children:[d.jsx("p",{className:"text-gray-700 mb-4",children:`"Mohari's commitment to sustainability is truly inspiring. Their products are not only stylish but also made with care for the environment."`}),d.jsxs("div",{className:"flex items-center",children:[d.jsx("img",{src:er,alt:"Customer",className:"w-10 h-10 rounded-full mr-4 object-cover"}),d.jsxs("div",{children:[d.jsx("h4",{className:"font-kalam text-lg",children:"Sarah Lee"}),d.jsx("p",{className:"text-gray-500 text-sm",children:"California, USA"})]})]})]}),d.jsxs("div",{className:"bg-white p-6 rounded-lg shadow-md",children:[d.jsx("p",{className:"text-gray-700 mb-4",children:`"I love the quality and comfort of Mohari's clothing. It's clear that they put a lot of thought into their designs and materials."`}),d.jsxs("div",{className:"flex items-center",children:[d.jsx("img",{src:er,alt:"Customer",className:"w-10 h-10 rounded-full mr-4 object-cover"}),d.jsxs("div",{children:[d.jsx("h4",{className:"font-kalam text-lg",children:"Michael Brown"}),d.jsx("p",{className:"text-gray-500 text-sm",children:"New York, USA"})]})]})]}),d.jsxs("div",{className:"bg-white p-6 rounded-lg shadow-md",children:[d.jsx("p",{className:"text-gray-700 mb-4",children:'"Fantastic customer service and beautiful products. I feel good knowing that my purchases are supporting a brand that cares about the planet."'}),d.jsxs("div",{className:"flex items-center",children:[d.jsx("img",{src:er,alt:"Customer",className:"w-10 h-10 rounded-full mr-4 object-cover"}),d.jsxs("div",{children:[d.jsx("h4",{className:"font-kalam text-lg",children:"Jessica Wong"}),d.jsx("p",{className:"text-gray-500 text-sm",children:"Toronto, Canada"})]})]})]})]})]})})]}),d.jsx(xt,{})]}),mL=[{id:1,name:"Duvet Product 1",price:"29.99",category:"duvet",image:"path/to/duvet1.mp4"},{id:2,name:"Bedding Product 2",price:"49.99",category:"bedding",image:"path/to/bedding2.mp4"},{id:3,name:"Mattress Product 3",price:"99.99",category:"mattress",image:"path/to/mattress3.mp4"},{id:4,name:"Pillow Product 4",price:"19.99",category:"pillow",image:"path/to/pillow4.mp4"},{id:5,name:"Sheets Product 5",price:"39.99",category:"sheets",image:"path/to/sheets5.mp4"},{id:6,name:"Duvet Product 6",price:"29.99",category:"duvet",image:"path/to/duvet6.mp4"},{id:7,name:"Bedding Product 7",price:"49.99",category:"bedding",image:"path/to/bedding7.mp4"},{id:8,name:"Mattress Product 8",price:"99.99",category:"mattress",image:"path/to/mattress8.mp4"},{id:9,name:"Pillow Product 9",price:"19.99",category:"pillow",image:"path/to/pillow9.mp4"},{id:10,name:"Sheets Product 10",price:"39.99",category:"sheets",image:"path/to/sheets10.mp4"},{id:11,name:"Duvet Product 11",price:"29.99",category:"duvet",image:"path/to/duvet11.mp4"},{id:12,name:"Bedding Product 12",price:"49.99",category:"bedding",image:"path/to/bedding12.mp4"},{id:13,name:"Mattress Product 13",price:"99.99",category:"mattress",image:"path/to/mattress13.mp4"},{id:14,name:"Pillow Product 14",price:"19.99",category:"pillow",image:"path/to/pillow14.mp4"},{id:15,name:"Sheets Product 15",price:"39.99",category:"sheets",image:"path/to/sheets15.mp4"},{id:16,name:"Duvet Product 16",price:"29.99",category:"duvet",image:"path/to/duvet16.mp4"},{id:17,name:"Bedding Product 17",price:"49.99",category:"bedding",image:"path/to/bedding17.mp4"},{id:18,name:"Mattress Product 18",price:"99.99",category:"mattress",image:"path/to/mattress18.mp4"},{id:19,name:"Pillow Product 19",price:"19.99",category:"pillow",image:"path/to/pillow19.mp4"},{id:20,name:"Sheets Product 20",price:"39.99",category:"sheets",image:"path/to/sheets20.mp4"}],gL=()=>{var p;const{currentUser:t,userData:e}=Ri(),[n,r]=D.useState((e==null?void 0:e.fullName)||""),[i,s]=D.useState((e==null?void 0:e.email)||""),[o,l]=D.useState(!1),[c,u]=D.useState("");D.useEffect(()=>{e&&e.fullName&&r(e.fullName),e&&e.email&&s(e.email)},[e]);const f=async g=>{g.preventDefault(),u("Updating...");try{const v=ht(tt,"users",t.uid);await Rx(v,{fullName:n,email:i}),u("Profile updated successfully!"),l(!1)}catch(v){console.error("Error updating profile:",v),u("Failed to update profile.")}};return d.jsxs("div",{className:"flex flex-col min-h-screen",children:[d.jsx("div",{className:"container mx-auto px-4 py-8 pt-[25px]"}),d.jsxs("main",{className:"flex-grow bg-gray-100 p-8",children:[d.jsx("h2",{className:"text-3xl font-kalam text-[#6F3A19] mb-6",children:"My Profile"}),d.jsxs("div",{className:"flex flex-col md:flex-row",children:[d.jsxs("section",{className:"md:w-1/2 md:pr-8 mb-8 md:mb-0",children:[d.jsx("h3",{className:"text-2xl font-hedvig text-[#6F3A19] mb-4",children:"Personal Information"}),d.jsxs("form",{onSubmit:f,className:"space-y-4 bg-white p-6 rounded shadow",children:[d.jsxs("div",{className:"flex items-center space-x-4",children:[d.jsx("img",{src:er,alt:"Profile",className:"w-24 h-24 rounded-full object-cover"}),d.jsx("button",{type:"button",onClick:()=>alert("Change Photo Functionality Coming Soon!"),className:"px-4 py-2 bg-[#6F3A19] text-white rounded hover:bg-[#5a3216]",children:"Change Photo"})]}),d.jsxs("div",{children:[d.jsx("label",{htmlFor:"fullName",className:"block text-gray-700",children:"Full Name"}),d.jsx("input",{type:"text",id:"fullName",name:"fullName",className:"w-full px-3 py-2 border rounded",placeholder:"John Doe",value:n,onChange:g=>r(g.target.value),disabled:!o,required:!0})]}),d.jsxs("div",{children:[d.jsx("label",{htmlFor:"email",className:"block text-gray-700",children:"Email"}),d.jsx("input",{type:"email",id:"email",name:"email",className:"w-full px-3 py-2 border rounded",placeholder:"john.doe@example.com",value:i,onChange:g=>s(g.target.value),disabled:!o,required:!0})]}),t&&!t.isAnonymous&&d.jsxs("div",{children:[d.jsx("label",{htmlFor:"password",className:"block text-gray-700",children:"Password"}),d.jsx("input",{type:"password",id:"password",name:"password",className:"w-full px-3 py-2 border rounded",placeholder:"Enter new password",disabled:!o})]}),o?d.jsxs(d.Fragment,{children:[d.jsx("button",{type:"submit",className:"w-full bg-[#6F3A19] text-white py-2 rounded hover:bg-[#5a3216]",children:"Save Changes"}),d.jsx("button",{type:"button",onClick:()=>l(!1),className:"w-full mt-2 bg-gray-300 text-gray-700 py-2 rounded hover:bg-gray-400",children:"Cancel"})]}):d.jsx("button",{type:"button",onClick:()=>l(!0),className:"w-full bg-blue-500 text-white py-2 rounded hover:bg-blue-600",children:"Edit Profile"}),c&&d.jsx("div",{className:`mt-4 text-center ${c.includes("failed")?"text-red-500":"text-green-500"}`,children:c})]})]}),d.jsxs("section",{className:"md:w-1/2 md:pl-8",children:[d.jsx("h3",{className:"text-2xl font-hedvig text-[#6F3A19] mb-4",children:"Account Details"}),d.jsxs("div",{className:"bg-white p-6 rounded shadow",children:[d.jsxs("p",{children:[d.jsx("strong",{children:"Account Type:"})," ",t!=null&&t.isAnonymous?"Guest (Anonymous)":"Registered User"]}),d.jsxs("p",{children:[d.jsx("strong",{children:"Member Since:"})," ",((p=e==null?void 0:e.createdAt)==null?void 0:p.toDate().toLocaleDateString())||"N/A"]})]})]})]})]}),d.jsx(xt,{})]})};let wl;const yL=new Uint8Array(16);function vL(){if(!wl&&(wl=typeof crypto<"u"&&crypto.getRandomValues&&crypto.getRandomValues.bind(crypto),!wl))throw new Error("crypto.getRandomValues() not supported. See https://github.com/uuidjs/uuid#getrandomvalues-not-supported");return wl(yL)}const Ke=[];for(let t=0;t<256;++t)Ke.push((t+256).toString(16).slice(1));function _L(t,e=0){return Ke[t[e+0]]+Ke[t[e+1]]+Ke[t[e+2]]+Ke[t[e+3]]+"-"+Ke[t[e+4]]+Ke[t[e+5]]+"-"+Ke[t[e+6]]+Ke[t[e+7]]+"-"+Ke[t[e+8]]+Ke[t[e+9]]+"-"+Ke[t[e+10]]+Ke[t[e+11]]+Ke[t[e+12]]+Ke[t[e+13]]+Ke[t[e+14]]+Ke[t[e+15]]}const wL=typeof crypto<"u"&&crypto.randomUUID&&crypto.randomUUID.bind(crypto),g_={randomUUID:wL};function mf(t,e,n){if(g_.randomUUID&&!e&&!t)return g_.randomUUID();t=t||{};const r=t.random||(t.rng||vL)();if(r[6]=r[6]&15|64,r[8]=r[8]&63|128,e){n=n||0;for(let i=0;i<16;++i)e[n+i]=r[i];return e}return _L(r)}const EL=()=>{const{cartItems:t,removeFromCart:e,updateQuantity:n,clearCart:r}=vm(),{currentUser:i,userData:s}=Ri(),o=Gn(),[l,c]=D.useState(1),[u,f]=D.useState({fullName:(s==null?void 0:s.fullName)||"",email:(s==null?void 0:s.email)||"",pincode:"",city:"",state:"",district:"",street:"",country:"India"}),[p,g]=D.useState("Prepayment"),[v,C]=D.useState({}),[N,k]=D.useState([]),[T,E]=D.useState(null),[R,O]=D.useState(!1),[F,V]=D.useState(""),[I,w]=D.useState(null),[_,x]=D.useState(null),[P,A]=D.useState(!1),[S,te]=D.useState("");D.useEffect(()=>{if(!i){let B=localStorage.getItem("guestUserId");B||(B=mf(),localStorage.setItem("guestUserId",B)),x(B)}},[i]),D.useEffect(()=>{(async()=>{if(i)try{const q=ht(tt,"users",i.uid),Y=await Ai(q);if(Y.exists()){const Z=Y.data();Z.addressInfo&&w(Z.addressInfo)}}catch(q){console.error("Error fetching saved address:",q)}})()},[i]);const ne=async B=>{const{name:q,value:Y}=B.target;if(f(Z=>({...Z,[q]:Y})),q==="pincode")if(/^\d{6}$/.test(Y)){O(!0),V("");try{const oe=await(await fetch(`https://api.postalpincode.in/pincode/${Y}`)).json();if(oe[0].Status==="Success")if(oe[0].PostOffice.length===1){const ye=oe[0].PostOffice[0];f(Me=>({...Me,city:ye.District,state:ye.State,district:ye.District})),E(ye),k([])}else oe[0].PostOffice.length>1&&(k(oe[0].PostOffice),f(ye=>({...ye,city:"",state:"",district:""})),E(null));else V("Invalid pincode entered. Please check and try again."),f(ye=>({...ye,city:"",state:"",district:""})),k([]),E(null)}catch(Z){console.error("Error fetching pincode data:",Z),V("Failed to fetch pincode data. Please try again later."),f(oe=>({...oe,city:"",state:"",district:""})),k([]),E(null)}finally{O(!1)}}else f(Z=>({...Z,city:"",state:"",district:""})),V("Pincode must be 6 digits"),k([]),E(null)},le=B=>{const q=B.target.value,Y=N.find(Z=>Z.Name===q);Y&&(E(Y),f(Z=>({...Z,city:Y.District,state:Y.State,district:Y.District})),k([]))},St=B=>{B.preventDefault();const q={};u.fullName||(q.fullName="Full name is required"),u.email?/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(u.email)||(q.email="Invalid email format"):q.email="Email is required",u.pincode||(q.pincode="Pincode is required"),u.city||(q.city="City is required"),u.state||(q.state="State is required"),u.district||(q.district="District is required"),u.street||(q.street="Street address is required"),u.country||(q.country="Country is required"),C(q),Object.keys(q).length===0&&c(3)},$=async B=>{var Z,oe;g(B),A(!0),te("");const q=t.reduce((ye,Me)=>ye+Me.price*Me.quantity,0)+599,Y={cartItems:t,address:u,paymentMethod:B,totalAmount:q,createdAt:xe.now(),status:"Processing"};try{const ye=i?i.uid:_,Me=[];for(const Oe of t){const Kn=await ca(Oe.productId);Kn?((Z=Kn.variants.find(bs=>bs.id===Oe.variantId))==null?void 0:Z.stock)<Oe.quantity&&Me.push({...Oe,reason:`Only ${(oe=Kn.variants.find(bs=>bs.id===Oe.variantId))==null?void 0:oe.stock} left in stock.`}):Me.push({...Oe,reason:"Product does not exist."})}if(Me.length>0){let Oe=`Some items in your cart are not available in the desired quantity:
